...
YD.on("finished", (err, data) => {
    ...
    const fnpath = path.join(__dirname, '..', process.env.FILE_DIR, data.title + ".mp3");
    if (fs.existsSync(fopath))
        fs.renameSync(fopath, fnpath);
    //Update status for record in DB
    Video.findOneAndUpdate({ videoId: data.videoId }, { status: true, videoName: data.title }, (err2, data2) => {
...
YD.on("progress", (data) => {
    ...
    io.to(data.videoId).emit('download-progress', { videoId: data.videoId, progress: data.progress.percentage });
});
